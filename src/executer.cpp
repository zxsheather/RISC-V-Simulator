...
void ExecuterModule::work() {
  ...
  if (in_rs && revert == 0) {
    std::cerr << "Executer: Appending PC " << std::hex << std::setw(8) << std::setfill('0')
              << to_unsigned(pc_rs) << std::dec << std::endl;
...
bool ExecuterModule::exec(max_size_t head) {
  ...
  switch (ops[head]){
    ...
    case Opcode::SRA: {
      std::cerr  << "Executer: SRA" << std::endl;
      if(rs1s[head] < 0&&rs2s[head]>31){
        rs_value <= 0xffffffff;
      }else{
        rs_value <= (rs1s[head] >> rs2s[head]);
      }
    ...
    case Opcode::SLTU: {
      ...
      rs_value <= ((uint32_t)rs1s[head] < (uint32_t)rs2s[head]);
      break;
    }
    ...
    case Opcode::SRAI: {
      std::cerr  << "Executer: SRAI" << std::endl;
      if(rs1s[head] < 0&&as[head]>31){
        rs_value <= 0xffffffff;
      }else{
        rs_value <= (rs1s[head] >> as[head]);
      }
    ...
    case Opcode::SLTIU: {
      ...
      rs_value <= ((uint32_t)rs1s[head] < (uint32_t)as[head]);
      break;
    }
    ...
    case Opcode::BLTU: {
      ...
      to_rs_flag = 0;
      memory_predict <= pcs[head];
      if((uint32_t)rs1s[head] < (uint32_t)rs2s[head]) {
        if (jumps[head] == 0) {
          revert_flag = 1;
        }
        memory_jump <= 1;
